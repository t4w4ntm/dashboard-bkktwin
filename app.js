...
function setProgressBar(fillId, aqiValue) {
  ...
  var percentage = Math.min(100, (aqiValue / 300) * 100); // Scale to 300 for Thai standard visual
  var offset = maxLen - (percentage / 100) * maxLen;

  // กำหนด class สำหรับสีตาม AQI level (Thai Standard)
  el.classList.remove('very-good', 'good', 'moderate', 'usg', 'unhealthy', 'very-unhealthy', 'hazardous');

  if (aqiValue <= 25) {
    el.classList.add('very-good');
  } else if (aqiValue <= 50) {
...
var THINGSPEAK = {
  ...
  thon_pm25: { channelId: "3192372", readKey: "ZWT3K5EV765AJITU", field: 1 },
  bang_pm25: { channelId: "3192391", readKey: "862VYY6T19BO3KLK", field: 1 },

  // Temperature per district (°C)
  klong_temp: { channelId: "3027679", readKey: "4M306YRQZ87072KV", field: 4 },
  thon_temp: { channelId: "3192372", readKey: "ZWT3K5EV765AJITU", field: 2 },
  bang_temp: { channelId: "3192391", readKey: "862VYY6T19BO3KLK", field: 2 },
...
// ====== AQI Calculator (Thai Standard) ======
function aqiFromPM25(pm) {
  // Thai AQI Breakpoints:
  // 0-25 µg/m³ -> AQI 0-25
  // 26-37 µg/m³ -> AQI 26-50
  // 38-50 µg/m³ -> AQI 51-100
  // 51-90 µg/m³ -> AQI 101-200
  // >90 µg/m³ -> AQI >200
  var bp = [
    [0.0, 25.0, 0, 25],
    [25.1, 37.0, 26, 50],
    [37.1, 50.0, 51, 100],
    [50.1, 90.0, 101, 200],
    [90.1, 600.0, 201, 600], // Extrapolated upper bound
  ];
...
function pollThingSpeakPM() {
  ...
  Promise.all([Promise.all(promises), Promise.all(waqiPromises)]).then(function () {
    ...
    districts.forEach(function (district) {
      ...
      // Treat negative values as invalid (e.g. -1 from sensor error)
      if (tsPM !== null && tsPM < 0) {
        tsPM = null;
      }
...
function pollThingSpeakTemp() {
  ...
  Promise.all([Promise.all(promises), Promise.all(owmPromises)]).then(function () {
    ...
    districts.forEach(function (district) {
      ...
      // Treat invalid values as null (e.g. -1 from sensor error)
      if (tsTemp !== null && tsTemp < 0) {
        tsTemp = null;
      }
...
function computeAQIAdvice(aqi) {
  if (aqi <= 25) return { label: 'Very Good', className: 'very-good', text: 'อากาศดีมาก เหมาะสำหรับกิจกรรมกลางแจ้งทุกประเภท' };
  if (aqi <= 50) return { label: 'Good', className: 'good', text: 'อากาศดี สามารถทำกิจกรรมกลางแจ้งได้ตามปกติ' };
  if (aqi <= 100) return { label: 'Moderate', className: 'moderate', text: 'เริ่มมีผลกระทบต่อสุขภาพ ควรเฝ้าระวังสุขภาพ' };
  if (aqi <= 200) return { label: 'Unhealthy', className: 'unhealthy', text: 'มีผลกระทบต่อสุขภาพ ควรลดระยะเวลาการทำกิจกรรมกลางแจ้ง' };
  return { label: 'Very Unhealthy', className: 'very-unhealthy', text: 'มีผลกระทบต่อสุขภาพมาก ควรงดกิจกรรมกลางแจ้ง' };
}
